...
function loadComponent(id, url, callback) {
  ...
  fetch(url, {
    cache: 'force-cache'  // Force use browser cache
  })
    .then(res => {
...
function initNavbar() {
  ...
  navList.querySelectorAll('li').forEach(li => {
    ...
    if (dropdown && link) {
      ...
      link.addEventListener('click', function(e) {
        // If link is # or mobile device, prevent default behavior and toggle dropdown
  ...
  // Improved external click handling
  document.addEventListener('click', function(e) {
    // Ensure menu exists and is loaded
...
function initDarkMode() {
  ...
  // Return updateDarkMode function for external calls
...
function initTeamShowcase() {
  ...
  function initExpandAnimations() {
    ...
    const expandObserver = new IntersectionObserver((entries) => {
      ...
      threshold: 0.3, // Lower threshold, trigger earlier
...
// Initialize sliding animation for image section
function initSlidingAnimation() {
  const animationSection = document.querySelector('.animation-image-section');
  if (!animationSection) return;
  
  const slidingImages = animationSection.querySelectorAll('.sliding-image');
  if (slidingImages.length === 0) return;
  ...
  const animationObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const section = entry.target;
        const images = section.querySelectorAll('.sliding-image');
        ...
        images.forEach((image, index) => {
          if (!prefersReducedMotion) {
            ...
            // For reduced motion preference, just show images
  ...
  // Start observing the animation section
  animationObserver.observe(animationSection);
}

document.addEventListener('DOMContentLoaded', function() {
  // Early application of dark mode to body, avoid page flicker
  try {
  ...
  function checkAndApplyDarkMode() {
    if (componentsLoaded.footer) {
      if (darkModeUpdater && componentsLoaded.menu) {
        // Use unified dark mode update function
        const isDark = document.body.classList.contains('dark-mode');
        darkModeUpdater(isDark);
      } else {
        // Apply directly to footer
